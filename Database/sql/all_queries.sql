...
BEGIN  
	...
	select CAST(json_extract(j1, '$.id') AS UNSIGNED) AS id, user.name, user.email, json_extract(j1, '$.date_begin') AS date_begin, json_extract(j1, '$.date_end') AS date_end, 
		JSON_PRETTY(REPLACE(json_extract(j1, '$.idCategory'),'"','')) AS categories, json_extract(j1, '$.date_requested') AS date_requested, IF ( LENGTH(json_extract(j1, '$.idCategory')) <= 5, JSON_PRETTY(CONCAT('[',
